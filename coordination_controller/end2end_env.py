...
from comm.algorithms.workspace import generate_targets_inside_workspace, read_polygon_workspace, plot_polygon_workspace, \
    check_point_in_polygon
from learn_deformation import tf_energy_difference, tf_target_config, np_target_config, tf_distribute_activation, \
    tf_activation_to_muscle_force, get_rod_parameter, make_rod, make_basis, test_rod, generate_straight, cal_arm
...
class CoordinationEnv():
    def __init__(self, num_targets, obs_num_targets, ground_range, penalty_coeff, bonus_coeff, termination_step,
                 state_type, energy_coeff, dist_coeff, dist_tol,
                 simulation_model, simulation_dir, render_plot=False, online=True, render_dir="frames/"):
        self.num_targets = num_targets
        self.obs_num_targets = obs_num_targets
        self.render_dir = render_dir
        self.crawl_amount = np.array([0.2775787711143494, 0.0, 0.0])  # when transverse muscle are all activated
        self.arm_length = np.array([1.0, 0.0, 0.0])
        self.penalty_coeff = penalty_coeff
        self.bonus_coeff = bonus_coeff
        self.render_plot = render_plot
        self.ep = 0
        self.hull_pts, self.polygon, self.view_pts, self.view_polygon = read_polygon_workspace(simulation_dir,
                                                                                               view=True)
        ...
        rod, _, self.max_force_list, self.muscle_radius_ratio_list = make_rod()

        self.rod_list = get_rod_parameter(rod)
        self.straight_activation_coeff = generate_straight(eager=False)[0]
        self.straight_activation = np_target_config(self.straight_activation_coeff[np.newaxis, :])

        self.energy_coeff = energy_coeff
        self.dist_coeff = dist_coeff
        self.dist_tol = dist_tol

        if np.isclose(self.penalty_coeff, 0.0):
            self.cal_reward = lambda: (- self.dist_coeff * self.dist) / 2.0 + self.bonus_coeff
        else:
            self.cal_reward = lambda: (
                                                  self.n_target_collected - self.energy_coeff * self.energy / 300.0 - self.dist_coeff * self.dist) / 2.0 + self.bonus_coeff  # if new_reach_flag else -self.penalty_coeff
            #
    ...
    def cal_state(self):
        ...
        self.state = np.zeros(self.obs_num_targets * 2 + 11 * 3)
        ...
        self.state[self.obs_num_targets * 2:] = self.curr_activation_coeff
        self.prev_activation_coeff = self.curr_activation_coeff
        self.prev_activation = self.curr_activation

    def crawl(self):
        self.base += self.crawl_amount
        self.pos += self.crawl_amount

    def step(self, current_action, pred_activation_coeff):
        self.action = current_action
        self.n_target_collected = 0
        self.dist = 0
        if current_action == 0:  # reach
            self.curr_activation_coeff = pred_activation_coeff[0]
            self.curr_activation = np_target_config(pred_activation_coeff)
            tip, arm = cal_arm(np.float32(self.curr_activation), self.rod_list, self.max_force_list, self.muscle_radius_ratio_list)
            arm = np.array(arm).T
            self.energy = 0
            #
            new_reach_flag = len(self.new_reached_list) > 0
            # if new_reach_flag:
            closest_target = self.make_cart_targets(self.state[:2])
            self.dist = np.linalg.norm(tip.numpy()[:2] / 0.2 - closest_target)
            # dist2all=np.linalg.norm(tip.numpy()[:2] / 0.2 - self.target_range[:,:2],axis=1)
            # closest_index= tf.argmin(dist2all)
            # dist=dist2all[closest_index]
            # print(dist)
            if self.dist < self.dist_tol:
                closest_index = np.linalg.norm(closest_target - (self.target_range - self.base)[:, :2], axis=1).argmin()
                self.reached_map[closest_index] = 1
                self.targets_eaten += 1
                self.n_target_collected = 1
            diff_activation = self.curr_activation - self.prev_activation
            clip_activation = np.maximum(diff_activation, np.zeros_like(diff_activation))
            self.energy = np.trace(np.matmul(clip_activation, np.transpose(clip_activation)))
            self.reward = self.cal_reward()
                #                       self.n_target_collected - self.energy_coeff * self.energy / 300.0 - self.dist_coeff * self.dist) / 2.0 + self.bonus_coeff  # if new_reach_flag else -self.penalty_coeff
            # w1 * diff_energy / 300.0 + w2 / 2 * distance ** 2
        elif current_action == 1:  # crawl
            arm = None
            self.crawl()
            self.energy = 99.0
            self.reward = (-self.energy_coeff * self.energy / 300.0) / 2.0 + self.bonus_coeff
            self.curr_activation_coeff = self.straight_activation_coeff
            self.curr_activation = self.straight_activation
        # print("reward",self.reward)
        self.total_energy += self.energy
        self.counter += 1
        if self.render_plot:
            self.render(current_action, arm)
    ...
    def collect_with_greedy(self):
        ...
        if len(self.new_reached_list) > 0:
            greedy_actions = list(np.setdiff1d(range(self.num_targets), self.new_reached_list))
            ...
            for _ in range(len(self.new_reached_list)):
                potential_activation, energy_cost_map = self.agent.env.cal_energy_cost_map(self.agent.env.activation,
                                                                                           remain_list)
                energy_cost_map[greedy_actions] = np.inf
                action = np.argmin(energy_cost_map)
                greedy_actions.append(action)
                diff_activation = np_target_config(potential_activation[action][np.newaxis, :]) - np_target_config(
                ...
                apply_activation = np_target_config(potential_activation[action][np.newaxis, :])
                # abs_energy = np.trace(np.matmul(apply_activation, np.transpose(apply_activation)))
                abs_energy = np.trace(np.matmul(clip_activation, np.transpose(clip_activation)))
    ...
    def plot_straight_arm(self):
        ...
        for k in range(100):
            self.ax.scatter(arm_x[k], arm_y[k], s=self.marker_size[k], color='mediumpurple', alpha=0.3)
    ...
    def plot_targets(self):
        ...
        for i in range(self.num_targets):
            ...
            if self.reachable_targets[i] == 1:
                ...
                plot_reachable = self.ax.scatter(target_list[i, 0], target_list[i, 1], marker='o', color="r",
                                                 label='target', s=300, linewidths=7, alpha=0.2)
            if self.reached_map[i] == 0:
                # plt.plot(target_list[i, 0], target_list[i, 1], 'bx', markersize=8, label="target%d" % i)
                plot_unreached = self.ax.scatter(target_list[i, 0], target_list[i, 1], marker='x', color="b",
                                                 label='target', s=300,
                                                 linewidths=7)
    ...
    def save_plot(self, current_action, energy, legend_list):
        ...
        label_list = ["workspace", "view range", "arm", "reachable targets", "unreached targets"]
        ...
        self.fig.savefig(self.render_dir + "/%03d_%03d" % (self.ep, self.render_counter), transparent=True)
        plt.close(self.fig)
        plt.close("all")
        self.fig, self.ax = plt.subplots(figsize=(16, 6))

    def render(self, current_action, arm=None):
        if current_action == 1:
            self.crawl_step += 1
        self.render_counter += 1
        if current_action == -1 or current_action == 1:
            plot1 = plot_polygon_workspace(self.hull_pts, x_shift=self.base[0], ax=self.ax)
            plot2 = plot_polygon_workspace(self.view_pts, x_shift=self.base[0], ax=self.ax)
            ...
            # # marker_size = np.linspace(150, 10, 100)
            plot3, = self.ax.plot(arm[0, :] / 0.2 + self.base[0], arm[1, :] / 0.2, 'mediumpurple', linewidth=2)
            for k in range(100):
                plt.scatter(arm[0, k] / 0.2 + self.base[0], arm[1, k] / 0.2, s=self.marker_size[k],
                            color='mediumpurple',
                            alpha=0.3)
            ...
            for i in self.new_reached_list:  # np.setdiff1d(self.new_reached_list, greedy_actions):
                self.ax.scatter(target_list[i, 0], target_list[i, 1], marker='x', color="b",
                                label='target', s=300,
                                linewidths=7)
            plot_reachable, plot_unreached = self.plot_targets()
            legend_list = [plot1, plot2, plot3, plot_reachable, plot_unreached]
            self.save_plot(current_action, self.energy / 300.0, legend_list)
            ...
            # legend_list = [plot1, plot2, plot3, plot_reachable, plot_unreached]
            # self.save_plot(current_action, energy=return_energy, legend_list=legend_list)
            # else:
            #     self.render_counter += 1
    ...
    def make_cart_targets(self, polar_target):
        rho, phi = polar_target.copy()
        phi *= np.pi
        x = rho * np.cos(phi)
        y = rho * np.sin(phi)
        return np.array([x, y])
    ...
    def make_polar_targets(self, obs_list, tip=[1.0, 0.0]):
        ...
        for target in obs_list:  # self.target_range:
            x, y, _ = target  # - self.base (no need to minus the base because tip and target are all absolute)
            pol_coor = cart2pol(x - tip_x, y - tip_y)
    ...
    def update_reachable_region(self):
        for i, target in enumerate(self.target_range):
            ...
            if inside_workspace:
                ...
                self.reachable_targets[i] = -1
                self.observable_targets[i] = 1 if check_point_in_polygon(target_relative, self.view_polygon) else -1

        self.reachable_index = np.where(self.reachable_targets == 1)[0]
        self.unreached_target_index = np.where(self.reached_map == 0)[0]
        self.new_reached_list = np.intersect1d(self.reachable_index, self.unreached_target_index)
    ...
    def generate_n_targets(self, from_n_map):
        if from_n_map > 0:
            ...
            if self.reward_type == "01sanity" or self.num_targets == 2:
                ...
                self.energy_mean = self.permutation_mean[random_draw] / (self.num_targets + 1)
